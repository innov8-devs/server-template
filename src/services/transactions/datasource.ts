...
class transactionsDatasource extends Base {
	async createWallet(userId: string) {
		const userWallet = await wallet.findOneBy({ userId: new ObjectId(userId) });
	...
	async walletToWalletTransfer({ recipient,amount, pinNumber }:walletToWalletTransferInput, sender: string) {
		const pinValid = await Pin.findOneBy({ userId: new ObjectId(sender), pin:pinNumber });
		if (!pinValid) return Promise.reject('Invalid pin');
		const fromUserWallet = await wallet.findOneBy({ userId: new ObjectId(sender) });
		const toUserWallet = await wallet.findOneBy({ userId: new ObjectId(recipient) });
